...
def maybe_reset_db():
    ...
    if last_reset is None or (datetime.datetime.now() - last_reset).total_seconds() > 60:
        ...
        cursor.execute("INSERT INTO flag VALUES (?);", ("flag{320562fcb2390d57}",))
...
def search():
    ...
    cursor = connection.cursor()
    try:
        assert not any(blocked in query.lower() for blocked in ("-", "/", "*", ";", "\\"))
...
if __name__ == '__main__':
    app.run(host="0.0.0.0", port=42007, debug=False)
